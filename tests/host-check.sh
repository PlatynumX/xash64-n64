...
python3 -m py_compile \
    ...
    "$ROOT/tests/test-r14-integration.py" \
...
python3 "$ROOT/tests/test-r14-integration.py"
...
grep -q 'ecda80fb9a29d45099a624344456eb3c7d01473d' "$ROOT/.github/workflows/build-r14.yml"
grep -q '35f85a0797324a5ed0c723203e33ab3c1da94fdd' "$ROOT/.github/workflows/build-r14.yml"

# Regression guards for the real current-Xash static-link route.
grep -q -- '--enable-static-binary' "$ROOT/scripts/build-xash-r14.sh"
grep -q -- '--static-linking=filesystem_stdio' "$ROOT/scripts/build-xash-r14.sh"
grep -q 'mips64-elf-' "$ROOT/scripts/build-xash-r14.sh"
grep -q 'TOOLWRAP' "$ROOT/scripts/build-xash-r14.sh"
grep -q 'xash-r14-waf-config.log' "$ROOT/scripts/build-xash-r14.sh"
grep -q "build/config.log" "$ROOT/scripts/build-xash-r14.sh" "$ROOT/.github/workflows/build-r14.yml"
grep -q 'Effective N64 target override' "$ROOT/scripts/apply-n64-port.py"
grep -q "LDFLAGS.*'-lc'.*'-ldragon'.*'-lm'.*'-ldragonsys'" "$ROOT/scripts/apply-n64-port.py"
grep -q 'omitting -lc leaves Waf configure tests' "$ROOT/scripts/apply-n64-port.py"
...
# the POSIX fallback. The r14 patcher must locate the fallback structurally.
