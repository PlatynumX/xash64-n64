# Current Xash upstream audit for r9

r9 intentionally changes only the architecture assumptions needed to create a real
Nintendo 64 target. The package audits current upstream before applying any edit.

Verified assumptions used by r9:
...
- Xash already has a low-memory mode, which r9 enables for the 8 MiB bring-up.
...
`engine/platform/n64`, r9 stops instead of silently mutating unfamiliar source.
