...
: > "$OUT/xash-r11-configure.log"
: > "$OUT/xash-r11-build.log"
...
./waf configure -T debug \
    ...
    --low-memory-mode=1 \
    --disable-rpath \
    --disable-werror 2>&1 | tee "$OUT/xash-r11-configure.log"
config_rc=${PIPESTATUS[0]}
set -e
if (( config_rc != 0 )); then
    echo "Xash r11 reached a real N64 configure frontier (exit $config_rc)." >&2
    exit "$config_rc"
fi

set +e
./waf build -j2 -v 2>&1 | tee "$OUT/xash-r11-build.log"
build_rc=${PIPESTATUS[0]}
set -e
if (( build_rc != 0 )); then
    echo "Xash r11 reached a real N64 compile/link frontier (exit $build_rc)." >&2
    exit "$build_rc"
fi

ELF=$(find build -type f -name xash -print -quit)
[[ -n "$ELF" && -f "$ELF" ]] || { echo "ERROR: Waf completed but no xash ELF was found" >&2; exit 1; }
cp "$ELF" "$OUT/xash64-n64-r11.elf"
"$ROOT/scripts/package-xash-rom.sh" "$OUT/xash64-n64-r11.elf" "$OUT/xash64-n64-r11.z64" | tee "$OUT/rom-packaging.log"
