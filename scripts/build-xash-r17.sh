...
: > "$OUT/xash-r17-configure.log"
: > "$OUT/xash-r17-build.log"
...
./waf configure -T debug \
    ...
    --low-memory-mode=1 \
    --disable-rpath \
    --disable-werror 2>&1 | tee "$OUT/xash-r17-configure.log"
...
if [[ -f build/config.log ]]; then
    cp build/config.log "$OUT/xash-r17-waf-config.log"
fi
...
if (( ${#conf_checks[@]} > 0 )); then
    tar -czf "$OUT/xash-r17-conf-checks.tar.gz" "${conf_checks[@]}"
fi

if (( config_rc != 0 )); then
    echo "Xash r17 captured the real N64 configure failure (exit $config_rc)." >&2
    if [[ -f "$OUT/xash-r17-waf-config.log" ]]; then
        echo "Full Waf configure log: xash-r17-waf-config.log" >&2
    fi
    exit "$config_rc"
fi

set +e
./waf build -j2 -v 2>&1 | tee "$OUT/xash-r17-build.log"
build_rc=${PIPESTATUS[0]}
set -e
if (( build_rc != 0 )); then
    echo "Xash r17 reached a real N64 compile/link frontier (exit $build_rc)." >&2
    exit "$build_rc"
fi

ELF=$(find build -type f -name xash -print -quit)
[[ -n "$ELF" && -f "$ELF" ]] || { echo "ERROR: Waf completed but no xash ELF was found" >&2; exit 1; }
cp "$ELF" "$OUT/xash64-n64-r17.elf"
"$ROOT/scripts/package-xash-rom.sh" "$OUT/xash64-n64-r17.elf" "$OUT/xash64-n64-r17.z64" | tee "$OUT/rom-packaging.log"
